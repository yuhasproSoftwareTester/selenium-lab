# Level 04 — Dropdown

## Mission

Before attempting this level, enter the unlock key from **Level 03**.
After unlocking, use Selenium to select the correct department from the dropdown.

**Correct Department:** `QA Automation`
**Requirement:** Use Selenium's **`Select` class**.

---

## Unlock Level 04

<style>
.selenium-game {
    font-size: 13px;
}

.selenium-game button {
    padding: 6px 11px;
    margin: 3px 4px 3px 0;
    border: 1px solid #cbd5e1;
    border-radius: 5px;
    background: #e5e7eb;
    color: #1f2937;
    font-size: 12px;
    cursor: pointer;
}

.selenium-game button:hover {
    background: #d1d5db;
}

.selenium-game .primary-button {
    background: #2563eb;
    color: white;
    border-color: #2563eb;
}

.selenium-game .primary-button:hover {
    background: #1d4ed8;
}

.selenium-game .reset-button {
    background: #dc2626;
    color: white;
    border-color: #dc2626;
}

.selenium-game .reset-button:hover {
    background: #b91c1c;
}

.selenium-game input,
.selenium-game select {
    width: 100%;
    max-width: 320px;
    padding: 6px 8px;
    margin: 3px 0 7px;
    border: 1px solid #cbd5e1;
    border-radius: 5px;
    font-size: 12px;
    box-sizing: border-box;
}

.selenium-game select {
    background: white;
    cursor: pointer;
}

.selenium-game .game-label {
    display: block;
    font-size: 12px;
    margin-top: 7px;
    margin-bottom: 3px;
}

.selenium-game .game-message {
    margin: 7px 0;
    font-size: 12px;
}

.selenium-game .success {
    color: #16a34a;
    font-weight: 600;
}

.selenium-game .error {
    color: #dc2626;
    font-weight: 600;
}

.selenium-game .key-box {
    display: none;
    max-width: 430px;
    margin-top: 10px;
    padding: 9px 11px;
    border-left: 3px solid #16a34a;
    background: #f0fdf4;
    font-size: 12px;
}

.selenium-game .key-box p {
    margin: 5px 0;
}

.selenium-game .key {
    font-family: monospace;
    font-weight: bold;
}

.selenium-game .challenge-locked {
    opacity: 0.45;
}
</style>

<div class="selenium-game">

<input
    type="text"
    id="levelKey"
    placeholder="Enter Level 03 Key"
    autocomplete="off"
>

<button
    type="button"
    id="unlockBtn"
    class="primary-button">
    Unlock Challenge
</button>

<button
    type="button"
    id="resetBtn"
    class="reset-button">
    Reset
</button>

<p
    id="unlockMsg"
    class="game-message">
</p>

</div>

---

## Challenge Playground

Select the correct department and submit the form.

<div
    id="challenge"
    class="selenium-game challenge-locked">

<label
    for="department"
    class="game-label">
    Department
</label>

<select
    id="department"
    name="department"
    disabled>

    <option value="">
        Select Department
    </option>

    <option value="development">
        Development
    </option>

    <option value="testing">
        Software Testing
    </option>

    <option value="qa">
        QA Automation
    </option>

    <option value="management">
        Management
    </option>

</select>

<button
    type="button"
    id="submitDepartment"
    class="primary-button"
    disabled>
    Submit
</button>

<p
    id="departmentMsg"
    class="game-message">
</p>

<div
    id="nextKey"
    class="key-box">

    <strong>Level 04 Completed</strong>

    <p>
        Congratulations! You completed Level 04.
    </p>

    <p>
        Use this key to unlock Level 05:
    </p>

    <span class="key">
        SEL-04-H3T7-M9Q2
    </span>

</div>

</div>

<script>
(function () {

    const LEVEL3_KEY =
        "SEL-03-R6N2-W8K5";

    const unlockBtn =
        document.getElementById("unlockBtn");

    const resetBtn =
        document.getElementById("resetBtn");

    const keyInput =
        document.getElementById("levelKey");

    const unlockMsg =
        document.getElementById("unlockMsg");

    const challenge =
        document.getElementById("challenge");

    const department =
        document.getElementById("department");

    const submitButton =
        document.getElementById("submitDepartment");

    const departmentMsg =
        document.getElementById("departmentMsg");

    const nextKey =
        document.getElementById("nextKey");


    function lockChallenge() {

        department.disabled = true;
        submitButton.disabled = true;

        challenge.classList.add(
            "challenge-locked"
        );
    }


    function unlockChallenge() {

        department.disabled = false;
        submitButton.disabled = false;

        challenge.classList.remove(
            "challenge-locked"
        );
    }


    lockChallenge();


    unlockBtn.addEventListener(
        "click",
        function () {

            const enteredKey =
                keyInput.value.trim();

            if (enteredKey === LEVEL3_KEY) {

                unlockChallenge();

                unlockMsg.className =
                    "game-message success";

                unlockMsg.textContent =
                    "Level 04 unlocked.";

            } else {

                lockChallenge();

                unlockMsg.className =
                    "game-message error";

                unlockMsg.textContent =
                    "Invalid key.";
            }
        }
    );


    submitButton.addEventListener(
        "click",
        function () {

            if (department.value === "qa") {

                departmentMsg.className =
                    "game-message success";

                departmentMsg.textContent =
                    "Correct department selected.";

                nextKey.style.display =
                    "block";

            } else {

                departmentMsg.className =
                    "game-message error";

                departmentMsg.textContent =
                    "Incorrect department. Try again.";

                nextKey.style.display =
                    "none";
            }
        }
    );


    resetBtn.addEventListener(
        "click",
        function () {

            keyInput.value = "";

            department.value = "";

            lockChallenge();

            unlockMsg.textContent = "";
            unlockMsg.className =
                "game-message";

            departmentMsg.textContent = "";
            departmentMsg.className =
                "game-message";

            nextKey.style.display =
                "none";
        }
    );

})();
</script>